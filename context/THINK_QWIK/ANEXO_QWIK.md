...
- **`createSignal()`**: Es una función de bajo nivel o una versión anterior de la API. **No debe ser utilizada directamente** por los desarrolladores para el estado de los componentes; `useSignal()` es la herramienta correcta.
...
- **`createComputed$()` / `ComputedFn`**: Son tipos o funciones internas. El desarrollador solo necesita interactuar con `useComputed$()`, pasándole una función que Qwik rastreará para sus dependencias.
...
### `event$` / `eventQrl`
- **Estado**: Son conceptos internos o nombres de API más antiguos.
- **Patrón Moderno**: El manejo de eventos se realiza a través de **props que terminan en `$`** (ej. `onClick$`, `onInput$`). El optimizador de Qwik convierte automáticamente la función pasada a estas props en un **QRL**.
...
### `sync$` / `_qrlSync`
- **Estado**: **APIs internas del framework**. No deben ser utilizadas por los desarrolladores.
- **Propósito**: Manejan la invocación síncrona de funciones, un patrón que va en contra del modelo de ejecución asíncrona y perezosa de Qwik. Su uso en el código de la aplicación se considera un anti-patrón.
...
- **`getQwikLoaderScript()`**:
    - **Definición**: Una función de `qwik/server` que devuelve el código JavaScript del **Qwik Loader** (~1KB) como una cadena de texto.
    - **Uso**: En una configuración de servidor manual, se llama a esta función para obtener el script y luego inyectarlo en la etiqueta `<head>` del documento HTML.

- **`getQwikPrefetchWorkerScript()`**:
    - **Definición**: Una función que devuelve el código del **Service Worker** responsable de la pre-carga especulativa de *bundles*.
    - **Uso**: Similar al anterior, se usa para implementar la estrategia de pre-carga de Qwik en un entorno de servidor personalizado.
